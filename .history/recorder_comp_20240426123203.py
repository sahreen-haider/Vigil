...
class Detect_verify:
    def capture_live_faces(self):
        ...
        while True:
            ...
            try:
                self.detected_face = DeepFace.extract_faces(self.frame, enforce_detection = False, detector_backend = model_names[3])
            ...
            if len(encodings) == 0: 
                ...
                if DeepFace.verify(np.array(encodings[f"candidate {counter-1}"]), self.frame, model_name = model_names[-1])["verified"] == False:
                    encodings.update({f"candidate {counter}" : self.frame.tolist()})
