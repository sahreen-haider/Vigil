...
class Detect_verify:
    ...
    def capture_live_faces(self):
        ...
        encodings = []                  # use dict instead if it moves slow 
        # also change .append() to .update()
        while True:
            ...
            if len(encodings) == 0: 
                encodings.append({f"candidate {counter}" : self.frame})
                counter += 1

            else:

                if DeepFace.verify(encodings[encodings.keys()[-1]], self.frame, model_name = model_names[-1])["verified"] == False:
                    encodings.append({f"candidate {counter}" : self.frame})
