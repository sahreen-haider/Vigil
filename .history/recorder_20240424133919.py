    ...
    def capture_live_faces(self):
        ...
        while True:
            ...
            if len(encoding_data) == 0: 
                encoding_data.update({f"candidate {counter}": self.final_detected_face.tolist(), f"Date {counter}":time_now[0], f"Time {counter}":time_now[1]})
                counter += 1
                ...
                if DeepFace.verify(np.array(encoding_data[f"candidate {counter-1}"]), self.frame, model_name = "VGG-Face")["verified"] == True:
                    ...
                    encoding_data.update({f"candidate {counter}": self.final_detected_face, f"Date {counter}":time_now[0], f"Time {counter}":time_now[1]})
