...
class Detect_verify:
    ...
    def capture_live_faces(self):
        ...
        while True:
            ...
            if len(encodings) == 0: 
                encodings.update({f"candidate {counter}" : self.frame.to_list()})
                counter += 1

            else:

                if DeepFace.verify(np.array(encodings[encodings.keys()[-1]]), self.frame, model_name = model_names[-1])["verified"] == False:
                    encodings.update({f"candidate {counter}" : self.frame.to_list()})
